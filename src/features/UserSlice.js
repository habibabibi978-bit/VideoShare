    ...
    try {
      ...
      if (!error.response) {
        ...
        if (error.code === 'ECONNABORTED') {
          return rejectWithValue({ message: 'Request timeout. Please check your connection and try again.' });
        } else if (error.message === 'Network Error') {
          return rejectWithValue({ message: 'Cannot connect to server. Please make sure the backend is running on http://localhost:3000' });
        }
